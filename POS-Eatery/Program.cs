    ...
    static class Program
    {
        ...
        static void Main()
        {
          ...
          //  MessageBox.Show(Environment.GetFolderPath(Environment.SpecialFolder.Startup).ToString());
            ...
            gclass.Expand_Database("CREATE TABLE  `db_pos_ayegun`.`table_loyalty` (`p_id` INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,`times` VARCHAR(255) NOT NULL,`amount` DECIMAL(65, 2) NOT NULL,`branch` VARCHAR(255) NOT NULL,`Code` VARCHAR(255) NOT NULL,`Date` TIMESTAMP ON UPDATE CURRENT_TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP) ENGINE = MYISAM; ALTER TABLE  `table_loyalty` ADD  `Registered_By` VARCHAR( 255 ) NOT NULL AFTER  `Code`");
           ...
           /* if (!Directory.Exists(Application.LocalUserAppDataPath + "/sola"))
            {
                Directory.CreateDirectory(Application.LocalUserAppDataPath + "/sola");
            ...
            try
            {
                MySqlConnection cn4 = new MySqlConnection(ConfigurationManager.ConnectionStrings["cnstring"].ConnectionString);
                cn4.Open();
                string query4 = "Select count(p_id) from table_sales_lump";
                MySqlCommand cmd4 = new MySqlCommand(query4, cn4);
                MySqlDataReader dr4 = cmd4.ExecuteReader();
                try
                {
                    if (dr4.Read())
                    {
                        if (Convert.ToInt32((string)dr4.GetValue(0).ToString()) >= 160)
                        {
                            proceed = false;
                        }
                        else
